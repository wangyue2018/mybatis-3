...
public class Reflector {
   ...
   * 解决 getting 冲突方法。
   * 最终，一个属性，只保留一个对应的方法。
   * <1> 处，基于返回类型比较。重点在 <1.1> 和 <1.2> 的情况，因为子类可以修改放大返回值，所以在出现这个情况时，选择子类的该方法
   *
  ...
  /**
   * <1> 处，解决冲突 setting 方法的方式，实际和 getting 方法的方式是不太一样的。首先，多的就是考虑了对应的 getterType 为优先级最高。
   * 其次，#pickBetterSetter(Method setter1, Method setter2, String property) 方法，选择一个更加匹配的，和 getting 方法是相同的，因为要选择精准的方法。
   *
   ...
   * 实际上，它是 #addGetMethods(...) 和 #addSetMethods(...) 方法的补充，因为有些 field ，不存在对应的 setting 或 getting 方法，所以直接使用对应的 field ，而不是方法。
   *
   * @param clazz
   */
  private void addFields(Class<?> clazz) {
    ...
    for (Field field : fields) {
      if (!setMethods.containsKey(field.getName())) {
        ...
        //返回int类型值表示该字段的修饰符(不同数字代表不同返回类型）
        int modifiers = field.getModifiers();
        //若 setMethods 不存在，则调用 #addSetField(Field field) 方法
        if (!(Modifier.isFinal(modifiers) && Modifier.isStatic(modifiers))) {
          addSetField(field);
        }
      }
      // 添加到 getMethods 和 getTypes 中
      // 若 getMethods 不存在，则调用 #addGetField(Field field) 方法
